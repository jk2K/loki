...
func (g *Gateway) FilterChunkRefs(ctx context.Context, req *logproto.FilterChunkRefRequest) (*logproto.FilterChunkRefResponse, error) {
	...
	for _, task := range tasks {
		...
		go consumeTask(task, tasksCh, resCh, errCh, logger)
	}
...
outer:
	for {
		...
		case task := <-tasksCh:
			level.Info(logger).Log("msg", "task finished", "task", task.ID)
			remaining--
			if remaining == 0 {
				break outer
			}
...
func consumeTask(task Task, tasksCh chan<- Task, resCh chan<- v1.Output, errCh chan<- error, logger log.Logger) {
	logger = log.With(logger, "task", task.ID)
	for {
		select {
		case err := <-task.errCh:
			level.Debug(logger).Log("msg", "got task error", "err", err)
			if task.ctx.Err() == nil {
				errCh <- err
			}
		case res := <-task.resCh:
			level.Debug(logger).Log("msg", "got partial result", "fp_int", uint64(res.Fp), "fp_hex", res.Fp, "chunks_to_remove", res.Removals.Len())
			if task.ctx.Err() == nil {
				resCh <- res
			}
		case <-task.Done():
